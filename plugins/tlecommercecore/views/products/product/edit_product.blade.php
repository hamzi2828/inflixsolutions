@php
    $categories = \Plugin\TlcommerceCore\Models\ProductCategory::with('childs', 'category_translations')
        ->where('parent', null)
        ->where('status', config('settings.general_status.active'))
        ->orderBy('id', 'DESC')
        ->select('name', 'parent', 'id')
        ->get();
    $brands = \Plugin\TlcommerceCore\Models\ProductBrand::with(['brand_translations'])
        ->orderBy('id', 'DESC')
        ->where('status', config('settings.general_status.active'))
        ->select('id', 'name')
        ->get();
    ...
    $tags = \Plugin\TlcommerceCore\Models\ProductTags::where('status', config('settings.general_status.active'))
        ->select('id', 'name')
        ->orderBy('id', 'DESC')
        ->get();
    ...
    $languages = \Core\Models\Language::where('status', config('settings.general_status.active'))
        ...
        ->get();
    $countries = \Plugin\TlcommerceCore\Models\Country::where('status', config('settings.general_status.active'))
        ->select('id', 'name', 'code')
        ->get();
    $attributes = \Plugin\TlcommerceCore\Models\ProductAttribute::with('attribute_translations')
        ...
        ->select('id', 'name')
        ->get();
    $product_collections = \Plugin\TlcommerceCore\Models\ProductCollection::with('products', 'collection_translations')
        ->where('status', config('settings.general_status.active'))
        ->select('id', 'name')
        ->get();
@endphp
                            ...
                            <select class="product-category-select form-control" name="categories[]" multiple>
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $product_details->product_categories->contains('category_id', $category->id) ? 'selected' : '' }}>
                                    ...
                                    @if (count($category->childs))
                                        @include(
                                            ...
                                            [
                                                ...
                                                'label' => 1,
                                                'parent' => null,
                                                'selected_categories' => $product_details->product_categories,
                                            ]
                                        )
                                    @endif
                                @endforeach
                        ...
                        <div class="col-sm-9">
                            <select class="product-brand-select form-control" name="brand">
                                ...
                                @foreach ($brands as $brand)
                                    <option value="{{ $brand->id }}" @if ($product_details->brand == $brand->id) selected @endif>
                        ...
                        <div class="col-sm-9">
                            <select class="product-unit-select form-control" name="unit">
                                @foreach ($units as $unit)
                                    <option value="{{ $unit->id }}" @if ($product_details->unit == $unit->id) selected @endif>
                    ...
                    <div class="form-row mb-20 @if (!empty($lang) && $lang != getdefaultlang()) area-disabled @endif">
                        ...
                        <div class="col-sm-9">
                            <select class="product-tags-select form-control" name="tags[]" multiple>
                                @foreach ($tags as $tag)
                                    <option value="{{ $tag->id }}"
                                        {{ $product_details->tags->contains('tag_id', $tag->id) ? 'selected' : '' }}>
                                        {{ $tag->name }}</option>
                                @endforeach
                            </select>
                            @if ($errors->has('tags'))
                        ...
                        <div class="col-sm-4">
                            <select class="theme-input-style" name="discount_amount_type">
                                <option value="{{ config('tlecommercecore.amount_type.flat') }}"
                                    @if ($product_details->discount_type == config('tlecommercecore.amount_type.flat')) selected @endif> {{ translate('Flat') }}</option>
            ...
            <div class="card mb-30 @if (!empty($lang) && $lang != getdefaultlang()) area-disabled @endif">
                ...
                <div class="card-body">
                    <div class="form-row mb-20">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="meta_title" class="theme-input-style"
                                placeholder="{{ translate('Type here') }}"
                                value="{{ $product_details->product_seo->meta_title }}">
                    ...
                    <div class="form-row mb-20">
                        ...
                        <div class="col-sm-9">
                            <textarea class="theme-input-style" name="meta_description">{{ $product_details->product_seo->meta_description }}</textarea>
                    ...
                    <div class="form-row mb-20">
                        ...
                        <div class="col-sm-">
                            @include('core::base.includes.media.media_input', [
                                'input' => 'meta_image',
                                'data' => $product_details->product_seo->meta_image,
                            ])
        ...
        <div class="col-lg-4 @if (!empty($lang) && $lang != getdefaultlang()) area-disabled @endif">
            ...
            <div class="card mb-30">
                ...
                <div class="card-body">
                    ...
                    @else
                        <p class="text text-danger mb-20">Please add a shipping profile, Otherwise customer can not
                            complete
                            checkout</p>
                    @endif

                    <p>You can add custom shipping area and shipping cost for specific area. Add custom shipping location <a
                            href="{{ route('plugin.tlcommercecore.shipping.configuration') }}"
                            class="btn-link">click</a> here.</p>
                </div>
            </div>
            <!--End Shipping area-->
            <!--Cash on delivery-->
                    ...
                    <div
                            ...
                            <div class="d-flex d-sm-inline-flex align-items-center mr-sm-5 mb-3">
                                <div class="custom-radio mr-3">
                                    <input type="radio" id="cod_in_custom_location"
                                        ...
                                        onchange="codLocation()" @if ($product_details->cod_location_type == config('tlecommercecore.cod_location.custom')) checked @endif>
                        ...
                        <div class="cod-contries_options">
                            @if (count($countries) > 0)
                                <div
                                    class="form-row mb-20 {{ $product_details->cod_location_type == config('tlecommercecore.shipping_location.custom') ? '' : 'd-none' }}">
                                    <label class="font-14 bold black col-sm-3">{{ translate('Countries') }} </label>
                                    <div class="col-sm-9">
                                        <select class="form-control cod-countries-select" name="cod_selected_countries[]"
                                            multiple onchange="codStateOptions()">
                                            @foreach ($countries as $country)
                                                <option value="{{ $country->id }}"
                                                    {{ $product_details->cod_countries != null && $product_details->cod_countries->contains('country_id', $country->id) ? 'selected' : '' }}>
                                                    {{ $country->name }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            @else
                                <div class="form-row mb-20">
                                    ...
                                    <a href="{{ route('plugin.tlcommercecore.shipping.locations.country.list') }}"
                                        target="_blank" class="m-2">{{ translate('Set location from here') }}</a>
                                </div>
                            @endif
                        </div>
                        <div class="cod-states-options">
                            @php
                                $states_options = \Plugin\TlcommerceCore\Models\States::where('status', config('settings.general_status.active'))->get();
                            @endphp
                            <div
                                class="form-row mb-20 {{ $product_details->cod_location_type == config('tlecommercecore.shipping_location.custom') ? '' : 'd-none' }}">
                                <label class="font-14 bold black col-sm-3">{{ translate('States') }} </label>
                                <div class="col-sm-9">
                                    <select class="form-control cod-state-select" name='cod_selected_states[]' multiple
                                        onchange="codCitiesOptions()">
                                        @foreach ($states_options as $state)
                                            <option value="{{ $state->id }}"
                                                {{ $product_details->cod_states != null && $product_details->cod_states->contains('state_id', $state->id) ? 'selected' : '' }}>
                                                {{ $state->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="cod-cities-options">
                            @php
                                $cities_options = \Plugin\TlcommerceCore\Models\Cities::where('status', config('settings.general_status.active'))->get();
                            @endphp
                            <div
                                class="form-row mb-20 {{ $product_details->cod_location_type == config('tlecommercecore.shipping_location.custom') ? '' : 'd-none' }}">
                                    ...
                                    <select class="form-control cod-city-select" name='cod_selected_cities[]' multiple>
                                        @foreach ($cities_options as $city)
                                            <option value="{{ $city->id }}"
                                                {{ $product_details->cod_cities != null && $product_details->cod_cities->contains('city_id', $city->id) ? 'selected' : '' }}>
                                                {{ $city->name }}
                                            </option>
                                        @endforeach
                                    </select>
            ...
            <div class="card mb-30">
                ...
                <div class="card-body">
                    <a href="{{ route('plugin.tlcommercecore.ecommerce.settings.taxes.list') }}"
                        class="btn-link">{{ translate('Create or manage taxes') }}</a>
                </div>
            </div>
            <!--End Vat & Tax-->
            <!--Warranty-->
                                ...
                                <div class="d-flex align-items-center">
                                    <label class="custom-checkbox position-relative mr-2">
                                        <input type="checkbox" id="{{ $collection->id }}" name="product_colletions[]"
                                            value="{{ $collection->id }}"
                                            {{ $collection->products->contains('product_id', $product_details->id) ? 'checked' : '' }}><span
...
@section('custom_scripts')
    ...
    <script>
        ...
        (function($) {
            ...
            /**
             * select product shipping countries
             * 
             */
            ...
            /**
             * select product shipping cities
             *
             */
            $('.city-select').select2({
                theme: "classic",
