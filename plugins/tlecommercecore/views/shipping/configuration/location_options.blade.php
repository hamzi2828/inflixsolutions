 @foreach ($countries as $country)
     <li class="cl-item cl-item-open">
         @php
             $states = \Plugin\TlcommerceCore\Models\States::where('country_id', $country->id)
                 ->select('id', 'name')
                 ->get();
         @endphp
         <div class="cl-label-wrap">
             @if (\Plugin\TlcommerceCore\Repositories\ShippingRepository::countryInAnotherZone($country->id, $profile__id))
                 ...
                 <label class="cl-label-title black" for="country-{{ $country->id }}"><span>{{ $country->name }}</span>
                 </label>
             ...
             @else
                 ...
                 <label class="cl-label-title black"
                     for="country-{{ $country->id }}"><span>{{ $country->name }}</span>
                 </label>
             @endif
         ...
         @if (count($states) > 0)
             <ul class="js-check-all-target js-check-all" data-check-all="country-{{ $country->id }}">
                 @foreach ($states as $state)
                     @php
                         $cities = \Plugin\TlcommerceCore\Models\Cities::where('state_id', $state->id)
                             ->select('id', 'name')
                             ->get();
                     @endphp
                     <li class="cl-item">
                         <div class="cl-label-wrap">
                             @if (\Plugin\TlcommerceCore\Repositories\ShippingRepository::stateInAnotherZone($state->id, $profile__id))
                                 ...
                                 @foreach ($cities as $city)
                                     <li class="cl-item">
                                         <div class="cl-label-wrap">
                                             ...
                                             @if (\Plugin\TlcommerceCore\Repositories\ShippingRepository::cityInAnotherZone($city->id, $profile__id))
