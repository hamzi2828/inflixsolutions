...
class ProductController extends Controller
{
    ...
    public function singleVariantInfo(Request $request)
    {
        try {
            ...
            $product_info = Product::where('id', $request->id)->select('discount_amount', 'discount_type')->first();
            if ($product_info->discount_amount > 0 && SettingsRepository::getEcommerceSetting('enable_product_discount') == 1) {
                if ($product_info->discount_type == config('tlecommercecore.amount_type.flat')) {
                    $discount = $product_info->discount_amount;
                } else {
                    $discount = ($variant_info->unit_price * $product_info->discount_amount) / 100;
                }
    ...
    {
        $categories = Cache::remember('mega-categories-' . session()->get('api_locale'), 60 * 60 * 24, function () {
            return ProductCategory::with(['category_translations', 'childs.childs:id,name,permalink,icon,parent,status'])
                ->where('parent', NULL)
                ->active()
                ->orderBy('id', 'ASC')
                ->get();
    ...
    public function searchSuggestions(Request $request)
    {
        try {
            $category_items = [];
            $product_items = [];
            $categories = ProductCategory::where('name', "like", "%$request->search_key%")->where('status', config('settings.general_status.active'))->get()->take(3);
            $products = Product::where('name', "like", "%$request->search_key%")->where('status', config('settings.general_status.active'))->get()->take(3);
            $tags = ProductTags::where('name', "like", "%$request->search_key%")->where('status', config('settings.general_status.active'))->select('name', 'permalink')->get()->take(5);
