...
class OrderRepository
{
     ...
     * @return Array 
     */
    public function statusWiseOrderCounter($status = null)
    {
        try {
            $data = [
                DB::raw('GROUP_CONCAT(DISTINCT(tl_com_ordered_products.order_id)) as order_id'),
            ...
            $total_order = 0;
            if ($status != null) {
                $total_order = DB::table('tl_com_ordered_products')
                    ->leftJoin('tl_com_orders', 'tl_com_orders.id', '=', 'tl_com_ordered_products.order_id')
                    ->groupBy('tl_com_ordered_products.order_id')
    ...
    public function shippingOptions($request)
    {
        try {
                        ...
                        if (sizeof($all_product_shipping_profiles) > 0) {
                            ...
                            foreach ($all_product_shipping_profiles as $key => $profile) {
                                ...
                                if (sizeof($validate_shipping_rates) > 0) {
                                    ...
                                    $tax = $this->calculateProductTax($profile['product']['id'], $profile['product']['unitPrice'], $profile['product']['quantity'], $request['location'], $profile['profile'], null);
                                    $temp = [
            ...
            if ($request->has('shipping_type')  && $request['shipping_type'] === 'pickup_delivery') {
                ...
                $product_list = json_decode($request['products'], true);
                foreach ($product_list as $product) {
                    $tax = $this->calculateProductTax($product['id'], $product['unitPrice'], $product['quantity'], null, null, $request['zone_id']);
                    $temp = [
                        'id' => $product['uid'],
                        'product' => $product,
                        'options' => [],
     ...
     * @return mixed 
     */
    public function calculateProductTax($product_id, $price = 0, $quantity = 1, $city_id = null, $profile_id = null,  $zone_id = null)
    {
        $tax_rate = 0;
        if ($zone_id != null) {
            $zoneInfo = ShippingZone::where('id', $zone_id)->first();
        } else {
            $zoneInfo = ShippingZone::where('profile_id', $profile_id)->whereIn('id', ShippingZoneCities::where('city_id', $city_id)->pluck('zone_id'))->first();
        }
        if ($zoneInfo != null) {
            $base_tax = $zoneInfo->base_tax;
            $collection_id = null;
            $state_id = null;
            $tax_info = null;

            $cityInfo = Cities::where('id', $city_id)->first();
            if ($cityInfo != null) {
                $state_id = $cityInfo->state_id;
            }
            $product_collection = CollectionHasProducts::where('product_id', $product_id)->first();

            //tax based on collection and state
            if ($product_collection != null && $state_id != null) {
                $collection_id = $product_collection->collection_id;
                $tax_info = ShippingZoneHasTaxes::where('product_collection_id', $collection_id)
                    ->where('zone_id', $zoneInfo->id)
                    ->where('state_id', $state_id)
                    ->first();
            }
            //Tax based on only collection
            if ($tax_info == null && $collection_id != null) {
                $tax_info = ShippingZoneHasTaxes::where('product_collection_id', $collection_id)
                    ->where('zone_id', $zoneInfo->id)
                    ->first();
            }

            //Tax based on only state
            if ($tax_info == null) {
                $tax_info = ShippingZoneHasTaxes::where('zone_id', $zoneInfo->id)
                    ->where('state_id', $state_id)
                    ->first();
            }
            //Get tax rate
            if ($tax_info != null) {
                $tax_rate = $tax_info->tax;
            } else {
                $tax_rate = $base_tax;
            }
        }
    ...
    public function getMatchingShippingRate($product_id, $rates, $price = 0, $quantity = 1)
    {
        ...
        $total_cbm = $this->calculateProductCBMOrTotalWeight($product_id, $quantity, true);
        $validated_rates = [];
    ...
    public function calculateProductCBMOrTotalWeight($product_id, $quantity, $weight = false)
    {
        ...
        if ($product_shipping_info != null) {
            if ($weight) {
                return $product_shipping_info->weight * $quantity / 1000;
            } else {
                $total_weight = $product_shipping_info->weight * $quantity / 1000;
                $cbm = ($product_shipping_info->height * $product_shipping_info->height * $product_shipping_info->height) / 1000000;
                $total_cbm = $cbm * $quantity;
                if ($total_weight > $total_cbm) {
                    return $total_weight;
                } else {
                    return $total_cbm;
                }
    ...
    public function guestCheckout($request)
    {
        try {
            ...
            if ($request->has('shipping_address')) {
                ...
                $customer_address->country_id = $shipping_address['country']['id'];
                $customer_address->state_id = $shipping_address['state']['id'];
                $customer_address->city_id = $shipping_address['city']['id'];
            ...
            $this->storeCouponUsageInfo($request, $order->id, $customer_id != null ? $customer_id : $guest_customer_id);
    ...
    public function storeOrderProducts($order_id, $products, $customer_id)
    {
        ...
        foreach ($products as $product) {
            ...
            $order_product->return_status = Product::where('id', $product['product_id'])->first()->is_refundable == config('settings.general_status.active') ? config('tlecommercecore.product_return_status.available') : config('tlecommercecore.product_return_status.not_available');
            $order_product->save();
    ...
    {
        if ($variant != null) {
            $variant_price = VariantProductPrice::where('product_id', $product_id)->where('variant')->first();
            if ($variant_price != null) {
    ...
    public function cancelOrder($order_id)
    {
        try {
            ...
            if ($order != null) {
                ...
                $order->save();
                $all_products = OrderHasProducts::where('order_id', $order_id)->get();
                ...
                if (auth('jwt-customer')->user() != null) {
                    ...
                    EcommerceNotification::sendCustomerOrderCancelNotification($order->id,  'An order has been cancelled');
                } else {
    ...
    /**
     * Change status of an item of order
     * 
     * @param Int $item_id
     * @param Int $order_id
     * @param Int $status
            ...
            if ($item != null) {
                if ($status_type == 'delivery_status') {
                    ...
                    if ($delivery_change_message != null) {
                        ...
                        if (auth('jwt-customer')->user() != null) {
                            //Send notification to admin
                            EcommerceNotification::sendCustomerOrderCancelNotification($order->id,  'An item removed from order');
                        } else {
                            //Send notification to customer
                            $btn_title = "Track Your Order";
    ...
    public function returnOrder($request)
    {
        try {
            if (isActivePluging('refund')) {
                ...
                //Send notification to admin
                $message = 'A product has been returned';
                EcommerceNotification::sendCustomerOrderReturnNotification($return_req->id, $message);
    ...
    {
        if (isActivePluging('refund')) {
            return \Plugin\Refund\Models\OrderReturnRequest::where('customer_id', $customer_id)->paginate($request['perPage']);
        } else {
            return [];
        }
    }
    ...
    public function getShippingLabelContent($order_id, $order_products)
    {
        try {
            ...
            foreach ($products as $product) {
                $total_order_discount += $product['order_discount'];
            ...
            if ($order_info->shipping_details != null) {
                $shipping_details = [
                    ...
                    'city' => $order_info->shipping_details->city->name,
                    'state' => $order_info->shipping_details->state->name,
                    'country' => $order_info->shipping_details->country->name,
                    'address' => $order_info->shipping_details->address,
                    'postal_code' => $order_info->shipping_details->postal_code,
                    'phone' => $order_info->shipping_details->phone_code . '' . $order_info->shipping_details->phone,
                ];
            }
    ...
    public function getInvoiceData($order_id, $order_products)
    {
        try {
            ...
            if ($order_info->billing_details != null) {
                ...
                $billing_info['phone'] = $order_info->billing_details['phone_code'] . '' . $order_info->billing_details['phone'];
                ...
                $billing_info['city'] = $order_info->billing_details->city->name;
                $billing_info['state'] = $order_info->billing_details->state->name;
                $billing_info['country'] = $order_info->billing_details->country->name;
            } else {
                $billing_info['name'] = $order_info->customer_info != null ? $order_info->customer_info->name : $order_info->guest_customer->name;
                $billing_info['phone'] = $order_info->customer_info != null ? $order_info->customer_info['phone_code'] . '' . $order_info->customer_info['phone'] : null;
                $billing_info['email'] = $order_info->customer_info != null ? $order_info->customer_info->email : $order_info->guest_customer->email;
