...
class ShippingRepository
{
    ...
    {
        return States::whereIn('id', Cities::whereIn('id', ShippingZoneCities::where('zone_id', $zone_id)->pluck('city_id'))->distinct('state_id')->pluck('state_id'))->get();
    }
    /**
     * Will update zone's base tax
     * 
     * @param Object $request
     * @return bool
     */
    public function updateZoneBaseTax($request)
    {
        try {
            DB::beginTransaction();
            $base_tax = $request['base_tax'] != null ? $request['base_tax'] : 0;
            $zone = ShippingZone::findOrFail($request['zone_id']);
            $zone->base_tax = $base_tax;
            $zone->save();
            DB::commit();
            return true;
        } catch (\Exception $e) {
