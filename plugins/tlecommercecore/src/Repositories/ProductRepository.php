...
class ProductRepository
{
     ...
     *@return Collections
     */
    public function productManagement($request)
    {

        $query = Product::with(['product_translations', 'variations', 'reviews', 'single_price', 'unit_info']);
    ...
    {
        return Product::with(['product_categories', 'tags', 'color_choices', 'choices', 'choice_options', 'single_price', 'variations', 'product_seo', 'shipping_info'])->findOrFail($id);
    }
    /**
    ...
    public function storeNewProduct($request)
    {
        ...
        try {
            ...
            //Store shipping info
            if ($free_shipping == config('settings.general_status.in_active')) {
    ...
    public function updateProduct($request)
    {
        try {
            if ($request['lang'] != null && $request['lang'] != 'en') {
                ...
                //Set shipping  profile of product
                if ($request->has('shipping_profile') && $request['shipping_profile'] != null) {
                    $product_profile = ShippingProfileProducts::firstOrCreate(['product_id' => $product->id, 'profile_id' => $request['shipping_profile']]);
                    $product_profile->profile_id = $request['shipping_profile'];
                    $product_profile->product_id = $product->id;
                    $product_profile->save();
                }
    ...
    public function storeProductVariantPrice($product_id, $request)
    {
        ...
        foreach ($request->variations as $variations) {
            $v_price = VariantProductPrice::firstOrCreate(['product_id' => $product_id, 'variant' => $variations['code']]);
    ...
    {
        $query = Product::with(['unit_info.unit_translations', 'single_price', 'variations', 'product_translations', 'reviews'])
