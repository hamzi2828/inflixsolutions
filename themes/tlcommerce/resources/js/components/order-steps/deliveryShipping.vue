  ...
  methods: {
    ...
    getShippingOptions() {
      ...
      if (this.isActivePickupPoint && !this.isActiveHomeDelivery) {
        this.$store.dispatch("setFinalShippingCost", 0);
        this.isActiveDeliveryOptions = false;
    ...
    getStates(origin) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            if (origin === "billing_info") {
              this.guestBillingInfo.states_options = response.data.states;
            } else if (origin === "shipping_info") {
              this.guestShippingInfo.states_options = response.data.states;
            }
          }
        })
        .catch((error) => {});
    ...
    getCities(origin) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            if (origin === "billing_info") {
              this.guestBillingInfo.cities_options = response.data.cities;
            } else if (origin === "shipping_info") {
              this.guestShippingInfo.cities_options = response.data.cities;
            }
          }
        })
        .catch((error) => {});
