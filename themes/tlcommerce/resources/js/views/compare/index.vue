...
export default {
  ...
  methods: {
    ...
    viewComparison() {
      let items = [];
      //first item
      let item_1 = this.products_suggestions_first.filter(
        (item) => item.name == this.search_key_1
      );
      if (item_1.length > 0) {
        items.push(item_1[0].id);
      }

      //last item
      let item_2 = this.products_suggestions_last.filter(
        (item) => item.name == this.search_key_2
      );
      if (item_2.length > 0) {
        items.push(item_2[0].id);
      }
