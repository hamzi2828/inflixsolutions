<template>
  <Dashboard>
    ...
    >
      <CModalHeader>
        <CModalTitle>{{ $t("Update Address Information") }}</CModalTitle>
        <button
          class="btn-circle bg-black size-35"
          @click="
            () => {
...
export default {
  ...
  methods: {
    ...
    saveAddress() {
      ...
      formData.append(
        "country",
        this.addressInfo.country ? this.addressInfo.country.id : ""
      );
      formData.append(
        "state",
        this.addressInfo.state ? this.addressInfo.state.id : ""
      );
      formData.append(
        "city",
        this.addressInfo.city ? this.addressInfo.city.id : ""
      );
      axios
        .post(
    ...
    getStates(origin) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            if (origin === "new") {
              this.addressInfo.states_options = response.data.states;
            } else if (origin === "edit") {
              this.editAddressInfo.states_options = response.data.states;
            }
          }
        })
        .catch((error) => {});
    ...
    getCities(origin) {
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success) {
            if (origin === "new") {
              this.addressInfo.cities_options = response.data.cities;
            } else if (origin === "edit") {
              this.editAddressInfo.cities_options = response.data.cities;
            }
          }
        })
        .catch((error) => {});
